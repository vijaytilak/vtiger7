		...
		<form class="form-horizontal contentsBackground" id="addFolder" method="post" action="index.php">
			...
			<div class="modal-body">
				<div class="row-fluid verticalBottomSpacing">
					...
					<span class="span7 row-fluid"><input data-validation-engine='validate[required]' id="foldername" name="foldername" style="width: 229px;" type="text" value="{vtranslate($FOLDER_MODEL->getName(), $MODULE)}"/></span>
				</div>
				<div class="row-fluid">
					<span class="span4">{vtranslate('LBL_FOLDER_DESCRIPTION', $MODULE)}</span>
					<span class="span7 row-fluid">
						<textarea style="width: 240px;" id="description" placeholder="{vtranslate('LBL_WRITE_YOUR_DESCRIPTION_HERE', $MODULE)}">{vtranslate($FOLDER_MODEL->getDescription(), $MODULE)}</textarea>
					</span>
				</div>
			</div>
            <div class="modal-footer">
                ...
                <button class="btn btn-success" type="button" id="saveFolderButton"><strong>Save</strong></button>
            </div>
		</form>
	</div>
{/strip}