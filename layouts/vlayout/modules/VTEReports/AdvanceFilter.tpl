...
{rdelim});
    ...
    function addGroupConditionGlue(groupIndex) {ldelim}
                ...
                var gcon_selectbox = '<select name="gpcon' + groupIndex + '" id="gpcon' + groupIndex + '" class="chzn-select" data-value="value" style="display: block;width:8em;" name="columnname" data-fieldinfo="">'+
                    ...
                    groupConditionGlueElement.innerHTML = '<div class="gconRow" style="border-top: 2px solid #ccc;width:100%;margin-top: 13px;"><div id="gconRow' + groupIndex +'" style="margin-top: -21px;" >'+gcon_selectbox+'</div>';
    ...
    function addConditionGroup(parentNodeId) {ldelim}
            ...
            newNode.innerHTML = "<div class='conditionList'><table class='small crmTable' border='0' cellpadding='5' cellspacing='0' width='100%' valign='top' id='conditiongrouptable_" + groupIndex + "' style='border:0px;' >" +
                    ...
                    "<button type='button' class='' style='float:left;' onclick='addNewConditionGroup(\"adv_filter_div\")'><strong>{vtranslate('LBL_NEW_GROUP',$MODULE)}</strong></button>" +
                    "<div style='float:left;'>&nbsp;&nbsp;</div>" +
                    "<button type='button' class='' style='float:left;' onclick='addConditionRow(\"" + groupIndex + "\")'><strong>{vtranslate('LBL_NEW_CONDITION',$MODULE)}</strong></button>" +
                    "</td>" +
                    "</tr>" +
    ...
    function deleteColumnRow(groupIndex, columnIndex) {
        ...
        if(parseInt(count_condition_row) == 2 && parseInt(groupIndex) > 1){
            ...
            if (document.getElementById(groupconditionglue_no))
            {
                ...
                if(parseInt(groupIndex) > 2 )this_parent_table.remove();
            }
        }
    }
	...
	node1.setAttribute('width', '25%');
	newNode.appendChild(node1);
    node1.innerHTML = '<select name="'+ctype+'groupcol'+columnIndex+'" id="'+ctype+'groupcol'+columnIndex+'" onchange="vtereports_updatefOptions(this, \''+ctype+'groupcolop'+columnIndex+'\');addRequiredElements(\''+ctype+'\','+columnIndex+');" class="chzn-select chzn-done" style="width:auto"><option value="none">{vtranslate('LBL_NONE',$MODULE)}</option></select>';
	...
	newNode.appendChild(node2);
	node2.innerHTML = '<div class="select-style"><select name="'+ctype+'groupop'+columnIndex+'" id="'+ctype+'groupop'+columnIndex+'" class="repBox" style="width:100%;" onchange="addRequiredElements(\''+ctype+'\','+columnIndex+');">'+
							'<option value="">{vtranslate('LBL_NONE',$MODULE)}</option>'+
							'{$FOPTION}'+
						'</select></div>';
