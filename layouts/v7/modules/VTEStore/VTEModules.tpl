...
{strip}
    ...
    <style>
        ...
        .module_short_description {
            ...
            width: 100%; height:150px;padding:2%;
        ...
        .icon-video {
            background:url("layouts/v7/modules/VTEStore/resources/images/video.png") no-repeat top center; display:inline-block;
    ...
    <div class="row-fluid">
        ...
        {else}
            {foreach item=STORECAT from=$STORECATS name=storecatgory}
                ...
                <div ><h3><u>{$STORECAT.store_category_name}</u></h3>{$STORECAT.store_category_desc}<br><br></div>
                <div class="clearfix"></div>
                <div class="row-fluid">
                ...
                {foreach item=VTEMODULE from=$VTEMODULES name=extensions}
                    <div class="col-md-4">
                        <div class="extension_container extensionWidgetContainer">
                            <div>
                                <div class="font-x-x-large boxSizingBorderBox" style="cursor:pointer">
                                    <table style="width: 100%">
                                        <tr>
                                            <td class="extension_header" id="{$VTEMODULE->module_name}Label"><h4>{$VTEMODULE->module_label}</h4></td>
                                            <td style="text-align: right">
                                            {if $VTEMODULE->userguide_link!=''}
                                                <a href="index.php?module=VTEStore&parent=Settings&view=Settings&mode=getModuleDetail&extensionId={$VTEMODULE->id}&extensionName={$VTEMODULE->module_name}&tab=Documentation" target="_blank" title="{vtranslate('Documentation', 'VTEStore')}"><img src="layouts/v7/modules/VTEStore/resources/images/user_manual_filled1600.png" id="{$VTEMODULE->module_name}Documentation" style="width: 25px; height: 25px;"/></a>
                                            {/if}
                                            {if $VTEMODULE->setting_url!='' && in_array($VTEMODULE->module_name,$VTMODULES)}
                                                <a href="{$VTEMODULE->setting_url}" target="_blank" title="{vtranslate('Extension settings', 'VTEStore')}"><img src="layouts/v7/modules/VTEStore/resources/images/setting.png" id="{$VTEMODULE->module_name}Settings" style="width: 25px; height: 25px;"/></a>
                                            {/if}
                                            </td>
                                        </tr>
                                    </table>
                            ...
                            <div>
                                <div class="row-fluid" style="overflow: hidden; background-color: #FAFAFA; max-height:175px">
                                    <a href="index.php?module=VTEStore&parent=Settings&view=Settings&mode=getModuleDetail&extensionId={$VTEMODULE->id}&extensionName={$VTEMODULE->module_name}" class="module_wrapper">
                                        ...
                                        {else}
                                            {assign var=imageSource value='layouts/v7/skins/images/unavailable.png'}
                                        {/if}
