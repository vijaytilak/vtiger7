...
{include file='ListViewActions.tpl'|@vtemplate_path:"VTEReports"}
							...
							<span class="actionImages">
                                {foreach item=LINK from=$LISTVIEW_ENTRY->getRecordLinks()}
                                    ...
                                    {if $link_type == "icon-trash"}
                                        <a class="deleteRecordButton vte_del_link vte_link" onclick="VTEReports_List_Js.listInstance.deleteVteReport('{$LISTVIEW_ENTRY->getId()}')">{vtranslate("Delete",$QUALIFIED_MODULE)}</a>
                                    {/if}
                                {/foreach}
							</span>
						</div>
