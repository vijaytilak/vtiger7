...
Vtiger.Class("Vtiger_Detail_Js",{
     ...
     * @param buttonElement
     */
    openQuickCreateActivity : function(buttonElement) {
        var instance = this;
        var elem = jQuery(buttonElement);
        var formParams = {
            activityType : elem.data('activity-type'),
            sourceModule : elem.data('source-module'),
            sourceRecordId : elem.data('source-record'),
        ...
        //Show quick create
        instance.showQuickCreateActivityForm(formParams,false);
    },
     ...
     * @param buttonElement
     */
    rescheduleActivity : function(buttonElement) {
        var instance = this;
        var elem = jQuery(buttonElement);
        var formParams = {
            activityRecordId : elem.data('activity-record'),
            activityType : elem.data('activity-type'),
            sourceModule : elem.data('source-module'),
            sourceRecordId : elem.data('source-record'),
            referenceModuleName : elem.data('ref-module'),
        ...
        if(formParams.subject.indexOf('[Reschedule') !== -1){
			//If Subject contains 'Reschedule'
        	var oldSubject = formParams.subject.match("(.*)Reschedule");
            var oldIndex = formParams.subject.match("Reschedule #(.*)]");
            var newIndex = ++oldIndex[1];
            formParams.subject = oldSubject[1] + 'Reschedule #' + newIndex +']';
		} else {
            formParams.subject = formParams.subject + ' [Reschedule #1]';
		}

        //Show quick create
        instance.showQuickCreateActivityForm(formParams,true);
    },

    /**
	 * Vijay
     * @param formParams
     * @param markAsRescheduled
     */
    showQuickCreateActivityForm : function(formParams,markAsRescheduled) {
        var thisInstance = this;
        var quickCreateNode = jQuery('#quickCreateModules').find('[data-name="'+ formParams.referenceModuleName +'"]');
        ...
        var customParams = {};
        customParams['parent_id'] = formParams.sourceRecordId;
        customParams['parentModule'] = formParams.sourceModule;
        ...
        app.event.on("post.QuickCreateForm.show",function(event,form){
            ...
            jQuery('<input type="hidden" name="sourceRecord" value="'+formParams.sourceRecordId+'" >').appendTo(form);
        ...
        app.event.on('post.QuickCreateForm.save',function(event,data){
            var params = {};
            params['record'] = formParams.sourceRecordId;
            params['view'] = 'Detail';
            params['module'] = formParams.sourceModule;
            ...
            app.request.post({"data":params}).then(
                function(err,data) {
                    ...
                    Vtiger_Detail_Js.getInstance().registerEventForActivityWidget();
                    if(markAsRescheduled) {
                        thisInstance.markAsRescheduled(formParams.activityRecordId,formParams.rescheduleReason);
					} else {
                        Vtiger_Detail_Js.getInstance().reloadActiveTab();
                        Vtiger_Detail_Js.getInstance().reloadHeader();
					}
                }
            );
        });
     ...
     * @param rescheduleReason
     */
	markAsRescheduled : function(activityRecordId,rescheduleReason) {
        var params = {
            module : "Calendar",
            action : "SaveFollowupAjax",
            mode : "markAsRescheduled",
        ...
        AppConnector.request(params).then(function(data){
            ...
            if(data.error){
                ...
                if(data.result.activitytype == 'Task') {
                    param = {text:app.vtranslate('Task Marked as Rescheduled')};
                } else {
                    param = {text:app.vtranslate('Event Rescheduled')};
                }
                app.helper.showSuccessNotification({'message' : param.text}); //can also use : showAlertBox
                ...
                param = {text:app.vtranslate('Unable to Reschedule Activity')};
                app.helper.showAlertBox({'message' : param.text});
            }
        });
	},
     ...
     * @param buttonElement
     */
    updateSalesStage : function(formParams) {

        var params = {
            module : "Potentials",
            action : "BasicAjax",
            mode : "updateSalesStage",
            newSalesStage : formParams.newSalesStage,
            record : formParams.opportunityRecordId
        };
        ...
        AppConnector.request(params).then(function(data){
            ...
            if(data.error){
                ...
                param = {text:app.vtranslate('Sales Stage Updated!')};
                app.helper.showSuccessNotification({'message' : param.text}); //can also use : showAlertBox
                ...
                param = {text:app.vtranslate('Unable to Update Sales Stage')};
                app.helper.showAlertBox({'message' : param.text});
            }
        });
     ...
     * @param formParams
     */
    checkProcessStageCompletion : function(buttonElement) {
        var instance = this;
        var elem = jQuery(buttonElement);
        var formParams = {
            opportunityRecordId : elem.data('opportunity-record'),
            currentSalesStage : elem.data('current-sales-stage'),
            newSalesStage : elem.data('new-sales-stage')
        };

        var params = {
            module : "Potentials",
            action : "BasicAjax",
            mode : "checkProcessStageCompletion",
            currentSalesStage : formParams.currentSalesStage,
            record : formParams.opportunityRecordId
        };
        ...
        AppConnector.request(params).then(function(data){
            ...
            if(data.error){
                ...
                app.helper.showConfirmationBox({'message' : text}).then(
                    ...
                    function(error, err){
                    	//No
                        param = {text:'Please fulfill the recommended actions and then proceed to the next stage!'};
                        app.helper.showAlertNotification({'message' : param.text});
                    }
                );
	...
	loadSelectedTabContents: function(tabElement, urlAttributes){
			...
			self.loadContents(url,urlAttributes,tabElement).then(function(data){
	...
	registerEventsForRelatedList : function(){
		...
		//Vijay
        this.registerRelatedRecordEditFromHeader();
        this.registerTempBtnHeader();
	...
	*/
	 ...
	 * @returns {unresolved}
	 */
	loadContents : function(url,data,tabElement){
		var thisInstance = this;
		var detailContentsHolder = this.getContentHolder();

        //Vijay - Tab functionality for the process div
        if(tabElement.data('linkKey') == 'process') {
            detailContentsHolder = jQuery('div.processDetails');
        } else {
            detailContentsHolder = jQuery('div.details');
		}
	...
	registerEventForActivityWidget : function(){
		...
		jQuery('.createActivity').on('click', function(e){
			...
			//Vijay - populate event quick create lookup with contact id
            var controller = app.controller();
            var contactId = controller.getFieldValue('contact_id');
			...
			app.event.on("post.QuickCreateForm.show",function(event,form){
				...
				//Vijay
                jQuery('<input type="hidden" name="contact_id" value="'+contactId+'" >').appendTo(form);

                var nextAction = jQuery(e.currentTarget).attr('data-next-action-type');
                jQuery('#Events_editView_fieldName_subject').val(nextAction);
	...
	//Vijay
    registerRelatedRecordEditFromHeader: function(){
        var self = this;
        var detailViewContainer = jQuery('.detailview-header');
        detailViewContainer.on('click', 'button[name="relationEdit"]', function(e) {
            e.stopImmediatePropagation();
            var element = jQuery(e.currentTarget);
            var editUrl = element.data('url');
            self.showOverlayEditView(editUrl);
    ...
    //Vijay
    registerTempBtnHeader: function(){
        var self = this;
        var detailViewHeader = jQuery('.detailview-header');
        detailViewHeader.on('click', 'i[name="reloadHeader"]', function(e) {
            e.stopImmediatePropagation();
            var element = jQuery(e.currentTarget);
            var reloadUrl = element.data('url');
            self.reloadHeader(reloadUrl);
        });
    },

    //Vijay
    reloadHeader: function(url){
    	if(!url) {
            var thisInstance = this;
            var elem = jQuery(".markAsCompleteBtn");
            // var sourceModule = elem.attr('sourceModule');
            // var sourceRecord = elem.attr('sourceRecord');
            var sourceRecord = thisInstance.getRecordId();
            var sourceModule = app.getModuleName();
            url = "index.php?module="+sourceModule+"&view=Detail&record="+sourceRecord+"&mode=showProcessView";
		}
        app.helper.showProgress();
        var postParams  = app.convertUrlToDataParams(url);

        app.request.post({data:postParams}).then(function(err,data){
            jQuery('.processDetails').html(data);
            app.helper.hideProgress();
        });
	...
	//Vijay
	reloadActiveTab: function($activeTab){
        var detailInstance = window.app.controller();
        var detailViewContainer = detailInstance.getDetailViewContainer();
	...
	registerEvents : function() {
		...
		//Vijay
        var recordId = jQuery("#recordId").val();
        var reloadUrl = "index.php?module=" + app.getModuleName() + "&view=Detail&record=" + recordId + "&mode=showProcessView";
        if(app.getModuleName()=='Potentials') {
            this.reloadHeader(reloadUrl);
		}
