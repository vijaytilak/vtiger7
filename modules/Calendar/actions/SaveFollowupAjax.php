...
class Calendar_SaveFollowupAjax_Action extends Calendar_SaveAjax_Action {
    ...
    function __construct() {
        ...
        //Vijay
	    $this->exposeMethod('markAsRescheduled');
    }
    ...
    public function markAsHeldCompleted(Vtiger_Request $request) {
        ...
        if($activityType == 'Task'){
            ...
            //Vijay - Remove validation : Future events cannot be marked as completed
/*            if($startDateTime > $currentDateTime){
	 ...
	 */
	public function markAsRescheduled(Vtiger_Request $request) {
		$moduleName = $request->getModule();
		$recordId = $request->get('record');
		$recordModel = Vtiger_Record_Model::getInstanceById($recordId,$moduleName);
		$recordModel->set('mode','edit');
		...
		if($activityType == 'Task'){
			...
			//set Deferred Reason Field
			$rescheduledReasonFieldName = 'cf_872';
			$recordModel->set($rescheduledReasonFieldName,$request->get('reschedulereason'));
		}
		else{
			$status = 'Rescheduled';
			$recordModel->set('eventstatus',$status);
			...
			//set Rescheduled Reason Field
			$rescheduledReasonFieldName = 'cf_870';
			$recordModel->set($rescheduledReasonFieldName,$request->get('reschedulereason'));
		}
